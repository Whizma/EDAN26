...
class Node(val index: Int) extends Actor {
  ...
  def receive = {
    ...
    case PushRequest(senderHeight, flow, currentEdge) => {
      ...
      if (senderHeight > h) {
        if (flow > 0) {
          val delta = min(flow, currentEdge.c - currentEdge.f)
          currentEdge += delta
          e += delta
        }
        else {
        ...
        sender ! PushResponse(true, edge, delta)
        if ((sink || source )) {
					control ! Bye()
				}
      } else {
        sender ! PushResponse(false, edge, 0)
      }
      exit("PushRequest")
    }
...
class Preflow extends Actor {
  ...
  def receive = {
    ...
    case ShouldTerminate => {
      excessSource = node(s) ! Excess
      excessSink = node(t) ! Excess
      if (abs(excessSource) == excessSink) {
        node(t) ! Excess
      }
    }
  }
